...
import {
  ...
  ignoreBankImportLine,
  matchBankImportLine,
  unmatchBankImportLine,
} from "@/server/actions/bank-imports";
...
type BankImportLine = Awaited<ReturnType<typeof getBankImportLines>>[number];
type Candidate = Awaited<ReturnType<typeof getReconciliationCandidates>>[number];
...
function LineRow({ line }: { line: BankImportLine }) {
  ...
  function loadCandidates() {
    startTransition(async () => {
      try {
        const result = await getReconciliationCandidates(line.id);
  ...
  function handleMatch(transactionId: string) {
    startTransition(async () => {
      await matchBankImportLine(line.id, transactionId);
      router.refresh();
    });
  }
  ...
  function handleIgnore() {
    startTransition(async () => {
      await ignoreBankImportLine(line.id);
      router.refresh();
    });
  }
  ...
  function handleUnmatch() {
    startTransition(async () => {
      await unmatchBankImportLine(line.id);
      router.refresh();
    });
  }
    ...
    <li className="rounded-control border border-pulso-border p-3">
      ...
      {candidates && (
        <ul className="mt-3 space-y-1">
          {candidates.length === 0 ? (
            <li className="text-xs text-slate-500">Nenhuma transação compatível encontrada.</li>
          ) : (
            candidates.map((c) => (
...
function ImportLines({ bankImportId }: { bankImportId: string }) {
  const [lines, setLines] = useState<BankImportLine[] | null>(null);
  const [isPending, startTransition] = useTransition();

  function load() {
    startTransition(async () => {
      const result = await getBankImportLines(bankImportId);
      setLines(result);
    });
  }
        ...
        <ul className="space-y-3">
          {imports.map((imp) => (
            <li key={imp.id} className="bg-white border border-slate-200 rounded-xl p-4">
              ...
              <ImportLines bankImportId={imp.id} />
            </li>
          ))}
        </ul>
      )}
