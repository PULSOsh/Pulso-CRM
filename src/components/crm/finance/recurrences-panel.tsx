...
import {
  ...
  deactivateFinancialRecurrence,
  generateNextRecurrenceOccurrence,
} from "@/server/actions/financial-recurrences";
...
export function RecurrencesPanel({ rules }: { rules: Rule[] }) {
  const router = useRouter();
  const [isPending, startTransition] = useTransition();
  const [creating, setCreating] = useState(false);
  const [targetType, setTargetType] = useState<"receivable" | "payable">("payable");
  ...
  function handleCreate(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        await createFinancialRecurrence({
          targetType,
          frequency,
          description,
  ...
  function handleGenerate(ruleId: string) {
    startTransition(async () => {
      try {
        await generateNextRecurrenceOccurrence(ruleId);
        router.refresh();
      } catch (err) {
  ...
  function handleDeactivate(ruleId: string) {
    startTransition(async () => {
      await deactivateFinancialRecurrence(ruleId);
      router.refresh();
    });
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h2 className="font-medium text-slate-900">Recorrências financeiras</h2>
      ...
      <p className="text-xs text-slate-500">
        Sem automação agendada nesta fase — gere a próxima ocorrência manualmente quando quiser
        lançar a parcela do período.
      </p>
      ...
      {creating && (
        ...
        >
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Select
              value={targetType}
              onChange={(e) => setTargetType(e.target.value as "receivable" | "payable")}
            >
          ...
          <Input
            placeholder="Descrição (ex.: Assinatura de software)"
            value={description}
          ...
          {rules.map((rule) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {rule.targetType === "payable" ? "Pagar" : "Receber"} ·{" "}
