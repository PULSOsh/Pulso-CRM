...
import { DashboardPreferencesPanel } from "@/components/crm/reports/dashboard-preferences-panel";
import { ExportCsvButton } from "@/components/crm/reports/export-csv-button";
import { getDashboardPreferences } from "@/server/actions/dashboard-preferences";
import {
  getCommercialReport,
  getFinancialReport,
...
export default async function ReportsPage({
  ...
  const [commercial, operational, financial, preferences] = await Promise.all([
  ...
  const isVisible = (key: string) => preferences.find((p) => p.key === key)?.isVisible ?? true;

  return (
    <AppShell active="reports" eyebrow="OPERAÇÃO" title="Relatórios">
      <div className="p-4 md:p-8 max-w-5xl mx-auto w-full space-y-8">
        <DashboardPreferencesPanel preferences={preferences} />
        ...
        {isVisible("commercial_section") && (
          <section className="bg-white border border-slate-200 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-lg text-slate-900">Comercial</h2>
              <ExportCsvButton report="commercial" days={days} />
            </div>
        ...
        {isVisible("financial_section") && (
          <section className="bg-white border border-slate-200 rounded-xl p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-lg text-slate-900">Financeiro</h2>
              <ExportCsvButton report="financial" days={days} />
            </div>
            <p className="text-sm text-slate-500 mb-4">
              {financial.totalReceivables} recebíveis gerados, total{" "}
              {currency(financial.totalAmount)}
            </p>
            <table className="w-full text-left text-sm">
              <thead className="text-slate-500 border-b border-slate-200">
                <tr>
